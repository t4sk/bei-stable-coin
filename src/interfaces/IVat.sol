...
interface IVat {
    ...
    function safes(bytes32 col_type, address safe) external view returns (Safe memory);
    // ilks
    function cols(bytes32 col_type) external view returns (CollateralType memory);
    // rely
    function add_auth(address user) external;
    // deny
    function remove_auth(address user) external;
    // wards
    function authorized(address owner, address user) external view returns (bool);
    // hope
